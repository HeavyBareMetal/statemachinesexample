...
{
    const PedLed pedLed;
    const RoadLed roadLed;
    const trafStates nextState;
    const bool bStoreCrossRqst:1;
    const uint32_t nextWaitInt;
};

constexpr StateData stateDataTable[] =
{
    ...
    {   PedLed::greenFl,RoadLed::red,       trafStates::pedRed,             1,  100,    },
    {   PedLed::red,    RoadLed::red,       trafStates::roadAmberToGreen,   1,  2000,   },
    {   PedLed::red,    RoadLed::amber,     trafStates::roadWaitGreen,      1,  5000,   },
...
{
    StateData* pData = &stateDataTable[(uint8_t)state];
    if (pData->bStoreCrossRqst && bCrossRqst) g_crossRqsted = true;
    ...
    if (bRunState)
    {
        setPedLed(pData->pedLed);
        setRoadLed(pData->roadLed);
        
        nextSwitchInterval = pData->nextWaitInt;
        ...
        if (bAdvNextState)  state = pData->nextState;
    }
}
