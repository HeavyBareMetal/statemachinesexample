...
void roadGoFunc(uint32_t timeMs, bool bCrossRqst);
void waitRoadAmberFunc(uint32_t timeMs, bool bCrossRqst);
void roadAmberFunc(uint32_t timeMs, bool bCrossRqst);
void roadRedToCrossFunc(uint32_t timeMs, bool bCrossRqst);
void pedGreenFunc(uint32_t timeMs, bool bCrossRqst);
void pedFlashGreenFunc(uint32_t timeMs, bool bCrossRqst);
void pedRedFunc(uint32_t timeMs, bool bCrossRqst);
void roadAmberToGreenFunc(uint32_t timeMs, bool bCrossRqst);

typedef void(*StateFunc)(uint32_t timeMs, bool bCrossRqst);
constexpr StateFunc stateFuncs[] =
{
    roadGoFunc,
    waitRoadAmberFunc,
    roadAmberFunc,
...
void loop() 
{
    ...
    stateFuncs[(uint8_t)state](timeMs, bCross);
}
